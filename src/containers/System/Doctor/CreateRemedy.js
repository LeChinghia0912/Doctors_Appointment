...
import { getBookingById, postCreateRemedy } from '../../../services/userService';
...
export default function CreateRemedy() {
    ...
    return (
        <LoadingOverlay active={isShowLoading} spinner={<ClimbingBoxLoader color={'#86e7d4'} size={15} />}>
            <div className="row">
                <div class="col-12">
                    <h5 className="">
            ...
            <div className="row">
                                    ...
                                    return (
                                        <div class="col-12 form-group">
                                            <div class="row align-item-center text-center">
                                                <div class="col-4">
                                                    {' '}
                                                    <input readonly type="text" value={drug.name} class="form-control" placeholder="" />
                                                </div>
                                                <div class="col-auto">
                                                ...
                                                <div class="col-1">
                                                    {' '}
                                                    <input onChange={(event) => handleOnchangeAmountDrug(event, drug.id)} type="text" value={drug.amount} class="form-control" />
                                                </div>
                                                <div class="col-4">
                                                    {' '}
                                                    <input
                                                        onChange={(event) => handleOnchangeDescriptionUsageDrug(event, drug.id)}
                                                        type="text"
                                                        value={drug.desciption}
                                                        class="form-control"
                                                        placeholder={language === 'en' ? 'Enter desription usage' : 'Nhập hướng dẫn sử dụng'}
                                                    />
                                                </div>
                                                <div class="col-1 d-flex align-items-center">
                                                    <i onClick={() => removeDrugFromTheList(drug.id)} class="fas fa-trash pointer text-red"></i>
                                                </div>
                                            </div>
                                        </div>
                                    );
                ...
                <div class="col-4">
                    <div class="row">
                        <div class="col-12">
                            ...
                            <input className="form-control" type="text" value={queryDrug} onChange={(event) => setQueryDrug(event.target.value)} />
                            <div class="mt-10">
                                <button type="button" class="btn btn-primary mr-5" onClick={() => handleFilterDrugs()}>
                                    <FormattedMessage id="medical-history.apply" />
                                </button>
                                <button type="button" class="btn btn-primary" onClick={() => handleResetDrugs()}>
                            ...
                            <ul class="list-group mt-10" style={{ overflowY: 'scroll', maxHeight: '400px' }}>
                                {listFilterDrugs.map((drug) => {
                                    return (
                                        <li onClick={() => handlePushDrugToList(drug)} class="list-group-item list-group-item-action d-flex justify-content-between align-items-center pointer">
                                            <span>{drug.name}</span> <i class="text-primary fas fa-plus-circle"></i>
                                        </li>
                                    );
                                })}
                            </ul>
                ...
                <div class="col-12 form-group">
                    <label>
                        <FormattedMessage id={'admin.manage-drug.more-descriptive-information'} />
                    </label>
                    <textarea className="form-control" aria-label="With textarea" value={desciption} onChange={(event) => handleOnChangeDescription(event)}></textarea>
                </div>
            </div>

            <button onClick={() => handleCreateRemedyImage()} type="button" class="btn btn-primary">
