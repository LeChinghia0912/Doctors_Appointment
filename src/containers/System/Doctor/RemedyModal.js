...
class RemedyModal extends Component {
    ...
    async componentDidMount() {
        if (this.props.dataModal && this.props.dataModal.imageRemedy && this.props.dataModal.email) {
            let base64data = new Buffer(this.props.dataModal.imageRemedy, 'base64').toString('binary');
            this.setState({
    ...
    async componentDidUpdate(prevProps, prevState, snapshot) {
        ...
        if (this.props.dataModal.imageRemedy !== prevProps.dataModal.imageRemedy) {
            if (this.props.dataModal && this.props.dataModal.imageRemedy && this.props.dataModal.email) {
                let base64dataImage = new Buffer(this.props.dataModal.imageRemedy, 'base64').toString('binary');
                this.setState({
            ...
            <Modal isOpen={isOpenModal} className={'booking-modal-container'} size="md" centered>
                <div className="modal-header">
                    <h5 className="modal-title">
                        <FormattedMessage id={'manage-patient.send-prescriptions-to-patients'} />
                    </h5>
                ...
                <ModalBody>
                    <div className="row">
                        ...
                        <div className="col-6 form-group">
                            <label>
                                <FormattedMessage id={'manage-patient.select-the-prescription-file'} />
                            </label>
                            {this.props.dataModal.imageRemedy && this.state.isShowBtnChooseOtherFile === true && (
                                <label>
                                    <FormattedMessage id={'manage-patient.there-is-a-pre-created-prescription-file'} />
                                </label>
                            )}
                            {this.props.dataModal.imageRemedy && this.state.isShowBtnChooseOtherFile === true && (
                                <button type="button" className="btn btn-primary" onClick={() => this.handleBtnChooseOtherFile()}>
                                    <FormattedMessage id={'manage-patient.choose-other-file'} />
                                </button>
                            )}
                            {(this.state.isShowChooseOtherFile === true || this.props.dataModal.imageRemedy === null) && (
